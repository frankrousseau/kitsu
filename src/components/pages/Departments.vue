...
const errors = reactive({ departments: false, edit: false, del: false })
const loading = reactive({ departments: false, edit: false, del: false })
...
const onLinkItem = data => {
  if (activeTab.value === 'linked-hardware') {
    store.dispatch('linkHardwareItem', {
      hardwareItemId: data.itemId,
      departmentId: data.departmentId
    })
    const item = hardwareItems.value.find(i => i.id === data.itemId)
    if (!linkedHardwareItems.value[data.departmentId]) {
      linkedHardwareItems.value[data.departmentId] = []
    }
    linkedHardwareItems.value[data.departmentId].push(item)
  } else if (activeTab.value === 'linked-software') {
    store.dispatch('linkSoftwareLicense', {
      softwareLicenseId: data.itemId,
      departmentId: data.departmentId
    })
    const item = softwareLicenses.value.find(i => i.id === data.itemId)
    if (!linkedSoftwareLicenses.value[data.departmentId]) {
      linkedSoftwareLicenses.value[data.departmentId] = []
    }
    linkedSoftwareLicenses.value[data.departmentId].push(item)
  }
}

const onUnlinkItem = data => {
  if (activeTab.value === 'linked-hardware') {
    store.dispatch('unlinkHardwareItem', {
      hardwareItemId: data.itemId,
      departmentId: data.departmentId
    })
    linkedHardwareItems.value[data.departmentId] = linkedHardwareItems.value[
      data.departmentId
    ...
    linkedSoftwareLicenses.value[data.departmentId] =
      linkedSoftwareLicenses.value[data.departmentId].filter(
        i => i.id !== data.itemId
      )
  }
}
