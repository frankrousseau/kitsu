...
export default {
  ...
  methods: {
    ...
    validationCellClicked (event) {
      ...
      if (this.isTaskSelected(task, columnId, assetId)) {
    ...
    isTaskSelected (task, columnId, assetId) {
      let isSelected = false
      if (task) {
        isSelected = task !== undefined &&
                     this.selectedTasks[task.id] !== undefined
      } else {
        const validationKey = `${columnId}_${assetId}`
